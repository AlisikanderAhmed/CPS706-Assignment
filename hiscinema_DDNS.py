#!/usr/bin/env python
#<------hisCinema DDNS File, Uses UDP to connect with Local DNS to resolve local DNS queries------>
...
def initRecords():
    connection = sqlite3.connect('HISCINEMA_Records.db')
    c = connection.cursor()

    c.execute('DROP TABLE IF EXISTS HISCINEMA_Records')
    c.execute('''CREATE TABLE HISCINEMA_Records (Name text, Value text, Type text)''')

    records =  [('herCDN.com', 'NSherCDN.com', 'NS'), ('NSherCDN', 'localhost', 'A'),
                ('hiscinema.com', 'NShiscinema.com', 'NS'), ('NShiscinema.com', 'localhost', 'A')]

    c.executemany('INSERT INTO HISCINEMA_Records VALUES (?,?,?)', records)
